...
for automated versioning and changelog generation. When creating commits, follow
these guidelines:
...
Your commit message type determines how the version is incremented:

- **Patch release (0.0.X)** - Bug fixes:

  ```md
  fix: correct calculation error in test simulator fix(auth): resolve token
  expiration issue
  ```
...
- **Minor release (0.X.0)** - New features (backwards compatible):
  ...
  ```md
  feat: add new test data generator feat(api): add support for async operations
  ```
...
- **Major release (X.0.0)** - Breaking changes:
  ...
  BREAKING CHANGE: authentication now requires API key instead of
  username/password
  ```
...
- `style:` - Code style changes that don't affect functionality (no version
  bump)
- `refactor:` - Code changes that neither fix bugs nor add features (no version
  bump)
...
1. Add `!` after the type: `feat!: new breaking feature`
2. Add `BREAKING CHANGE:` in the commit footer:

   ```md
   feat: update client interface

   BREAKING CHANGE: removed deprecated get_data() method, use fetch_data()
   instead
   ```
...
BREAKING CHANGE: API now requires token-based authentication"
```
