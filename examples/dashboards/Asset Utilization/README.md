# Asset Utilization Monitoring Example

## Overview:

This example project provides all required resources to configure and execute an Asset Utilization workflow
example. It includes a Jupyter Notebook and instructions for implementing a scheduled routine for data processing and
a Grafana dashboard designed to visualize asset utilization metrics.

## Solution Overview:

This example leverages a Grafana dashboard that utilizes built-in data sources. Asset-level
...
approach ensures that every asset is associated with utilization-specific tags, enabling straightforward
visualization through the Grafana dashboard using out-of-the-box data sources.

1. Asset Utilization
    - The Asset Utilization dashboard provides visibility into asset-level details. This includes additional
      information such as calibration status. The dashboard presents high-level metrics along with two time-series
      charts that visualize utilization trends for the selected assets across predefined categories.
...
6. After the notebook runs successfully at the scheduled time, tags will be generated for each asset.
   Navigate to **Systems Management >> Assets,** select an asset and open the **Tags** tab for viewing tags associated
   with the asset. You will see a new section of tags called Utilization. These tags will be updated every day with the
