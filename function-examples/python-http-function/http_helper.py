# From https://github.com/spinframework/spin-python-sdk/blob/e93a76aabf01ba0be190f72308a468bf5342492d/src/spin_sdk/http/__init__.py
...
from main_world.types import Ok, Err
from main_world.imports.types import (
...
from main_world.imports.streams import StreamError_Closed
...
from main_world.exports import IncomingHandler as Base
...
async def send_async(request: Request) -> Response:
    ...
    if headers_dict.get('content-length') is None:
        ...
        headers_dict = headers_dict.copy() # type: ignore
    ...
    while True:
        ...
        if chunk is None:
            headers_incoming = incoming_response.headers()
            simple_response = Response(
                incoming_response.status(),
                dict(map(
                    lambda pair: (pair[0], str(pair[1], "utf-8")),
                    headers_incoming.entries()
                )),
                bytes(body)
            )
            headers_incoming.__exit__(None, None, None)
